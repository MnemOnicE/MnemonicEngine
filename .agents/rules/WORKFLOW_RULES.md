...
The ingestion process is managed by `scripts/smart_ingest.py`, which is designed to run every 5 commits (or when the directory is empty), keeping only the latest 3 snapshots. It can also be forced via the `--force` flag for manual or emergency updates.
